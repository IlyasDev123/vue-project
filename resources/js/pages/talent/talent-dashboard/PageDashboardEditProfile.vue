        ...
        <form action="#" @submit.prevent="onSubmit">
          <fieldset class="role__fieldset fieldset__currentRole">
            <input
              ...
              v-model="data.current_role"
            />
            <div v-show="errors && !data.current_role" class="error_message">
              current role {{ errors }}
            </div>
          </fieldset>
          <fieldset
          ...
          >
              ...
              <div class="hidden_flex_holder">
                ...
                >
                  <input
                    ...
                    :checked="data.state_id == country.id"
                  />
            ...
            <div v-show="errors && !data.state_id" class="error_message">
              State {{ errors }}
            </div>
          </fieldset>
          <fieldset
...
export default {
  ...
  //     this.getSkillById(to.params.id);
  //     next();
  //   },
  ...
  methods: {
    async getProfile() {
      this.data = this.getTalentMeta;
    },

    async onSubmit() {
      if (!this.data.current_role) {
        return (this.errors = "is required.");
      }
      this.isDisable = true;
      let response = await TalentService.updateProfile(this.data).catch(
        (error) => {
