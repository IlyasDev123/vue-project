<template>
  <div class="d__flex d__flex-h-between d__flex-v-center">
    <ComponentTitle pageTitle="All Recruiter" />

    <Filter
      :status="filterStatus"
      :talent="employers"
      @passStatus="getRecruiterList"
    />
  </div>
  <div class="all__members">
    <!-- ADD New -->
    <router-link
      :to="{ name: 'AddRecruiterPage' }"
                ...
                >
                  <div class="member__area">
                    <div class="member__img">
                      <img :src="this.$public + list?.avatar" alt="" />
                    </div>
            ...
            <td>
              <div class="member__status" :class="bindStatus(list.status)">
                <select
                  ...
                  :class="bindClass(list?.is_blocked)"
                >
                  <option value="0" :selected="list.is_blocked === 0">
                    Active
                  </option>
                  <option value="1" :selected="list.is_blocked === 1">
                    Blocked
                  </option>
                </select>
                <span v-if="authProfile.type !== this.$constants.ADMIN">{{
                  statusRequired(list.status)
                }}</span>
              </div>
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    getPage(page) {
      this.getRecruiterList(this.selectStatus, page);
    },
    getRecruiterList(sendToStatus, page = null) {
      ...
      recruiterService
        .getRecruiterList(payload, pageParam)
        .then((data) => {
          this.employers = data.data.data.data;
