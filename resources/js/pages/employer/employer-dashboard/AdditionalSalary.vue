  ...
  >
    <div class="role__container">
      <h1 class="text__center">
        Additional details freeform box along <br />with Salary Range
      </h1>
          ...
          <div class="indutry__holder">
            <fieldset>
              <input
                ...
                placeholder="1000"
                v-model="salary"
                id="indutsry__Social-Media"
              />
              <label for="indutsry__Social-Media">$</label>
            </fieldset>
...
export default {
  ...
  created() {
    ...
    this.salary = formData ? formData.salary : null;
  },
  data() {
    return {
      salary: null,
  ...
  methods: {
    async onSubmit() {
      ...
      console.log(payload);
      let response = await chirrrpMeta.addSalary(payload);
      let data = response.data;
      if (data.status) {
        let payload = {
          salary_id: data.data.id,
          salary: this.salary,
        };
        this.$functions.setLocalStorage("mutlistepfm", payload);
        this.$router.push({ name: "EmployerAllSet" });
      } else {
...
<style>
.selct__industry.AdditionalSalary .indutry__holder input {
  position: static;
  opacity: 1;
  background: none;
  border: none;
...
.selct__industry.AdditionalSalary fieldset label {
  font-family: "Roboto";
  font-weight: 500;
  font-size: 20px;
  line-height: 103.19%;
...
.selct__industry.AdditionalSalary h1 {
  margin: 0 100px 100px;
  font-size:26px;
}
.selct__industry.AdditionalSalary  form {
...
.selct__industry.AdditionalSalary fieldset {
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.selct__industry.select__SpecificIndustry.AdditionalSalary .indutry__holder {
