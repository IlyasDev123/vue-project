      ...
      <div class="regsiter_form_holder">
        <form @submit.prevent="onSubmit">
          <div class="indutry__holder">
            <fieldset
              v-for="teamLovableAspect in this.$store.state.chirrrpMeta
                .chirrrpMeta.teamLovableAspects"
              :key="teamLovableAspect.id"
            >
              <input
                type="radio"
                name="id"
                :value="teamLovableAspect.id"
                v-model="id"
                :id="`employeer__${teamLovableAspect.name}`"
                :checked="teamLovableAspect.id == id"
                @change="onChange"
              />
              <label :for="`employeer__${teamLovableAspect.name}`">
                {{ teamLovableAspect.name }}</label
              >
            </fieldset>
          </div>
          <button class="big__blue__btn" type="submit">Save Changes</button>
        </form>
      </div>

      <div class="role__btn">
        <router-link
          :to="{ name: 'EmployerManagementStyle' }"
...
export default {
  ...
  created() {
    ...
    this.id = formData ? formData.team_loveable_aspect_id : null;
  },
  methods: {
    onChange() {
      let payload = {
        team_loveable_aspect_id: this.id,
      };
